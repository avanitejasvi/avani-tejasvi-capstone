...
### 6. Feedback — three channels, one real primary signal

The multi-tenant rebuild briefly assumed a calendar owner's own RSVP on their own event couldn't carry a signal (an event you merely *own* is auto-accepted with nothing actionable to read). That assumption turned out to be wrong for this project specifically: the single-user build's real, live history (see `BUILD_LOG.md`) shows genuine YES/NO/MAYBE + RSVP-comment feedback working, because the student was added as a real **attendee**, not just the calendar's owner — an attendee entry starts `needsAction` and has to be actively responded to, regardless of whose calendar it's on. `agent/react_agent.py`'s `create_event_with_id` now takes an `attendee_email` (the logged-in student's own email, threaded through from `agent/jobs/scheduling.py`), restoring exactly that mechanism per-student:

- **Automated, primary** (`agent/jobs/collect_feedback.py`, runs before each Monday's scheduling): for every unresolved past meal, reads the student's real Calendar RSVP (`GoogleCalendarSkill.get_response`) — a real `yes`/`no`/`maybe` plus whatever comment they added. Deleted/cancelled → treated as a decline. Genuinely hasn't responded yet (`needsAction`) → left unresolved, checked again on a later run rather than guessed at.
- **Manual, immediate** (`/feedback` page): a self-serve way to rate a past meal right now, without waiting for the weekly job — same underlying `apply_feedback` call, same sentiment classification, just triggered by a page visit instead of a Calendar RSVP.
- **Weekly, proactive** (`/checkin` page): asks about *this week's* actual uploaded menu directly, before the meal even happens — see the Multi-tenant section below.
- All three write through the same `MenuPreferenceMatchingSkill.apply_feedback` and the same sentiment classification (`GeminiSkill.interpret_feedback_note`, or the student's own bring-your-own key via `agent/feedback_sentiment.py`) — the same first-feedback-replaces-estimate, then-delta-adjusts logic as the single-user build, completely unchanged. Only *how* a response reaches that function differs per channel.
...
5. **Updates itself.** This property genuinely wobbled mid-rebuild, worth stating honestly: assuming a multi-tenant event's owner-RSVP couldn't carry a signal (necessary-*seeming* for multi-tenancy) initially left only a negative signal (`collect_feedback.py`'s cancellation check) — a preference model that could only ever get *more pessimistic*, never actually learn what a student liked. Re-checking that assumption against this project's own history (the single-user build's real RSVP feedback genuinely worked) showed the real fix was adding the student back as a real Calendar **attendee**, per-student — not giving up on RSVPs. Between the restored real RSVP, the manual `/feedback` page, and the proactive `/checkin` page, this property is fully intact again, in three complementary forms rather than one.
